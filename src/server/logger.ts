...
abstract class customLogger {
    buildLine(msg:string, color: colorType, err?: Error): string{
        ...
        const errName = color=="error"?(err? err.name : 'Unknown error'): "NO_ERROR"
...
class ConsoleLogger extends customLogger{
    log(msg: string, color: colorType, err?: Error){
        const line: string = this.buildLine(msg, color, err)
        console.log(line)
    }
}
...
export default class Logger{
    ...
    constructor(lvl: logType, _config?: logConfigurations){
        switch(lvl){
